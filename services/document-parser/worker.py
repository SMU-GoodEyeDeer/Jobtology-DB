...
def project_archive(path, parser):
    """Unpack document bundles; the existing library still parses each format."""
    ...
    with zipfile.ZipFile(path) as archive, tempfile.TemporaryDirectory(prefix="members-") as directory:
        ...
        for ordinal, entry in enumerate(files, 1):
            ...
            member = {"name": name, "ordinal": ordinal, "extension": ext}
            members.append(member)
            if ext not in {"pdf", "hwp", "hwpx", "doc", "docx"}:
            ...
            if not 0 < entry.file_size <= 64 * 1024 * 1024:
                ...
                continue
            # Never extract to a supplied member path or follow a member URL.
            target = Path(directory) / (str(ordinal) + "." + ext)
            ...
            if parts:
                offset += 2
            sections.append(dict(section_no=len(sections)+1, locator_kind="archive_member", locator=str(ordinal),
                                 kind="member_name", archive_member=name, start=offset, end=offset+len(heading),
            ...
            for section in output["sections"]:
                sections.append(section | {"section_no": len(sections)+1, "locator_kind": "archive_member_block",
                    "locator": str(ordinal)+":"+section["locator"], "archive_member": name,
                    "member_raw_hash": member["raw_hash"], "start": base+section["start"], "end": base+section["end"]})
            blocks.extend(block | {"archive_member": name} for block in output["semantic"]["blocks"])
            parts.append(chunk)
            offset += len(chunk)
    markdown = "\n\n".join(parts)
