-- Generated v2 serializer; shared native value helpers retain v1 behavior.
BEGIN;
CREATE OR REPLACE FUNCTION ontology.interchange_check_v2(choice text,preview boolean DEFAULT false)
...
BEGIN
 ...
 SELECT artifacts->'mappings/hop-v2.json' INTO mapping FROM ontology.interchange_contract WHERE version='hop-ontology-interchange-v2';
...
CREATE OR REPLACE FUNCTION ontology.export_jsonld_v2(choice text DEFAULT NULL,preview boolean DEFAULT false)
...
BEGIN
 id:=ontology.interchange_check_v2(choice,preview);
 SELECT * INTO r FROM ontology.corpus_release WHERE release_id=id;
 SELECT * INTO contract FROM ontology.interchange_contract WHERE version='hop-ontology-interchange-v2';
 mapping:=contract.artifacts->'mappings/hop-v2.json';
