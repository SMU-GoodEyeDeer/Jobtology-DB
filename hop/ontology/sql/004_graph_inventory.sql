-- Immutable, typed graph inventory. Sealing is not serving-release activation.
BEGIN;
CREATE TABLE IF NOT EXISTS ontology.graph_node (
...
CREATE TABLE IF NOT EXISTS ontology.graph_edge (
 release_id text NOT NULL REFERENCES ontology.corpus_release,
 edge_id text NOT NULL,
 subject_id text NOT NULL,
 predicate text NOT NULL CHECK(predicate ~ '^[A-Z][A-Z_]*$'),
 object_id text NOT NULL,
 properties jsonb NOT NULL CHECK(jsonb_typeof(properties)='object'),
 content_hash text NOT NULL,
 PRIMARY KEY(release_id,edge_id)
);
CREATE INDEX IF NOT EXISTS ontology_graph_subject ON ontology.graph_edge(release_id,subject_id);
CREATE TABLE IF NOT EXISTS ontology.graph_load (
 ...
 node_count bigint, edge_count bigint,
 error text
);
CREATE UNIQUE INDEX IF NOT EXISTS ontology_one_graph_writer ON ontology.graph_load(database_id) WHERE state='RUNNING';
CREATE TABLE IF NOT EXISTS ontology.activation_event (
 event_id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
 release_id text NOT NULL REFERENCES ontology.corpus_release,
 previous_release_id text REFERENCES ontology.corpus_release,
 actor text NOT NULL,
 reason text NOT NULL,
 created_at timestamptz NOT NULL DEFAULT clock_timestamp()
);
CREATE TABLE IF NOT EXISTS ontology.active_release (
 singleton boolean PRIMARY KEY DEFAULT true CHECK(singleton),
 release_id text NOT NULL REFERENCES ontology.corpus_release,
 event_id bigint NOT NULL REFERENCES ontology.activation_event
);
DO $$ DECLARE name text; BEGIN
 FOREACH name IN ARRAY ARRAY['graph_node','graph_edge','activation_event'] LOOP
...
BEGIN
 ...
 PERFORM 1 FROM ontology.corpus_release WHERE release_id=id AND state='PREPARING' AND manifest IS NULL FOR UPDATE;
 IF NOT FOUND THEN RAISE EXCEPTION 'ONTOLOGY_RELEASE_NOT_PREPARING_OR_ALREADY_SEALED'; END IF;
END $$;

-- Nested source maps (for example dates) become typed dotted property names.
-- Object arrays belong in structural graph nodes; they must not turn into JSON strings.
CREATE OR REPLACE FUNCTION ontology.flat_properties(value jsonb,prefix text DEFAULT '') RETURNS jsonb
LANGUAGE plpgsql IMMUTABLE AS $$
DECLARE result jsonb:='{}'::jsonb; item record; name text; kinds integer;
BEGIN
 FOR item IN SELECT * FROM jsonb_each(value) LOOP
  -- Escape literal dots/tilde in keys, including Q-Net's already-flat lineage
  -- names, so a literal key cannot collide with the nested-map separator.
  name:=prefix||replace(replace(item.key,'~','~0'),'.','~2');
  CASE jsonb_typeof(item.value)
   WHEN 'null' THEN CONTINUE;
   WHEN 'object' THEN result:=result||ontology.flat_properties(item.value,name||'.');
   WHEN 'array' THEN
    SELECT count(DISTINCT jsonb_typeof(x)) INTO kinds FROM jsonb_array_elements(item.value) x;
    IF kinds>1 OR EXISTS(SELECT 1 FROM jsonb_array_elements(item.value) x WHERE jsonb_typeof(x) NOT IN ('string','number','boolean'))
    THEN RAISE EXCEPTION 'GRAPH_PROPERTY_REQUIRES_HOMOGENEOUS_PRIMITIVES: %',name; END IF;
    result:=result||jsonb_build_object(name,item.value);
   ELSE result:=result||jsonb_build_object(name,item.value);
  END CASE;
 END LOOP;
 RETURN result;
END $$;

CREATE OR REPLACE FUNCTION ontology.graph_record_id(i ontology.input_record) RETURNS text LANGUAGE sql IMMUTABLE AS $$
 SELECT 'source-record/'||ontology.hash(jsonb_build_array(i.source_id,i.source_record_id,i.raw_sha256,i.locator,i.normalized_hash,i.field_lineage))
$$;

CREATE OR REPLACE FUNCTION ontology.graph_attachment_id(d ontology.artifact_document) RETURNS text LANGUAGE sql IMMUTABLE AS $$
 SELECT 'source-attachment/'||ontology.hash(jsonb_build_array(d.bundle_id,d.artifact_id))
$$;
CREATE OR REPLACE FUNCTION ontology.graph_document_part_id(d ontology.artifact_document,kind text,part jsonb) RETURNS text LANGUAGE sql IMMUTABLE AS $$
 SELECT ontology.graph_attachment_id(d)||'/'||kind||'/'||ontology.hash(jsonb_build_array(part->>'entry_name',part->>(kind||'_no')))
$$;
...
CREATE OR REPLACE FUNCTION ontology.verify_graph_numbers(id text) RETURNS void LANGUAGE plpgsql AS $$
BEGIN
 -- SQL predicates can be reordered. Keep casts inside CASE so source strings
 -- cannot reach numeric conversion before their value type has been checked.
 IF EXISTS(SELECT 1 FROM ontology.graph_property p WHERE p.release_id=id AND CASE WHEN p.value_type='integer'
  THEN p.text_value::numeric < -9223372036854775808::numeric OR p.text_value::numeric > 9223372036854775807::numeric ELSE false END)
 THEN RAISE EXCEPTION 'GRAPH_INTEGER_OUT_OF_RANGE'; END IF;
 IF EXISTS(SELECT 1 FROM ontology.graph_property p WHERE p.release_id=id AND CASE WHEN p.value_type='number'
  THEN (p.text_value::double precision)::text::numeric<>p.text_value::numeric ELSE false END)
 THEN RAISE EXCEPTION 'GRAPH_FLOAT_PRECISION_LOSS'; END IF;
END $$;
...
-- Native Hop sends one primitive/array member per row and batches with UNWIND.
...
SELECT release_id,owner_type,owner_id,content_hash,key,is_array,ordinal,value,array_size,
 ...
 WHEN jsonb_typeof(value)='string' AND (key IN ('date_posted','closing_date','established_date') OR key LIKE 'dates.%')
...
BEGIN
 ...
 PERFORM ontology.seal_graph(id);
 ...
 -- A historical success cannot certify the destination during/after a failed
 -- reload. Only this execution's complete native verification restores it.
 UPDATE ontology.corpus_release SET graph_verified_at=NULL WHERE release_id=id;
END $$;

CREATE OR REPLACE FUNCTION ontology.finish_graph_load(token text,nodes bigint,edges bigint,verified boolean) RETURNS void LANGUAGE plpgsql AS $$
DECLARE execution ontology.graph_load%ROWTYPE; r ontology.corpus_release%ROWTYPE;
BEGIN
 PERFORM retention.gate();
 SELECT * INTO STRICT execution FROM ontology.graph_load WHERE load_id=token FOR UPDATE;
 SELECT * INTO STRICT r FROM ontology.corpus_release WHERE release_id=execution.release_id FOR UPDATE;
 IF execution.state<>'RUNNING' THEN RAISE EXCEPTION 'GRAPH_LOAD_NOT_RUNNING'; END IF;
 IF NOT verified OR r.state IN ('FAILED','REVOKED') OR r.manifest_hash<>execution.manifest_hash OR
  nodes<>(r.manifest->>'nodes')::bigint OR edges<>(r.manifest->>'edges')::bigint OR
  r.manifest IS DISTINCT FROM ontology.graph_inventory_manifest(r.release_id)
 THEN RAISE EXCEPTION 'GRAPH_LOAD_VERIFICATION_FAILED'; END IF;
 UPDATE ontology.graph_load SET state='VERIFIED',finished_at=clock_timestamp(),node_count=nodes,edge_count=edges WHERE load_id=token;
 UPDATE ontology.corpus_release SET graph_verified_at=clock_timestamp() WHERE release_id=r.release_id;
 PERFORM retention.leave_writer(token);
END $$;
CREATE OR REPLACE FUNCTION ontology.fail_graph_load(token text) RETURNS void LANGUAGE plpgsql AS $$
BEGIN
 PERFORM retention.gate();
 UPDATE ontology.graph_load SET state='FAILED',finished_at=clock_timestamp(),error='Native workflow failed; inspect its log.' WHERE load_id=token AND state='RUNNING';
 IF FOUND THEN PERFORM retention.leave_writer(token); END IF;
END $$;

-- This explicit gate stays closed until the remaining ontology contracts exist.
-- A verified graph candidate must not silently become a complete serving corpus.
...
UNION ALL
SELECT p.release_id,'UNRESOLVED_POSTING_OUTCOME' FROM ontology.posting_selection p WHERE outcome<>'ACCEPTED';

CREATE OR REPLACE FUNCTION ontology.activate_release(id text,actor text,reason text) RETURNS void LANGUAGE plpgsql AS $$
DECLARE previous text; event bigint; r ontology.corpus_release%ROWTYPE;
BEGIN
 PERFORM retention.gate();
 IF ingestion.maintenance_active() THEN RAISE EXCEPTION 'RETENTION_ACTIVE'; END IF;
 IF nullif(btrim(actor),'') IS NULL OR nullif(btrim(reason),'') IS NULL THEN RAISE EXCEPTION 'ACTIVATION_PROVENANCE_REQUIRED'; END IF;
 SELECT * INTO STRICT r FROM ontology.corpus_release WHERE release_id=id FOR UPDATE;
 IF r.state NOT IN ('READY','SUPERSEDED','ACTIVE') OR r.graph_verified_at IS NULL OR
  EXISTS(SELECT 1 FROM ontology.publication_issue WHERE release_id=id) THEN RAISE EXCEPTION 'RELEASE_NOT_READY_FOR_ACTIVATION'; END IF;
 IF r.manifest IS DISTINCT FROM ontology.graph_inventory_manifest(id) THEN RAISE EXCEPTION 'SEALED_GRAPH_INVENTORY_CHANGED'; END IF;
 SELECT release_id INTO previous FROM ontology.active_release WHERE singleton FOR UPDATE;
 IF previous=id THEN RETURN; END IF;
 UPDATE ontology.corpus_release SET state='SUPERSEDED' WHERE state='ACTIVE';
 UPDATE ontology.corpus_release SET state='ACTIVE',activated_at=clock_timestamp() WHERE release_id=id;
 INSERT INTO ontology.activation_event(release_id,previous_release_id,actor,reason) VALUES(id,previous,actor,reason) RETURNING event_id INTO event;
 INSERT INTO ontology.active_release VALUES(true,id,event) ON CONFLICT(singleton) DO UPDATE SET release_id=excluded.release_id,event_id=excluded.event_id;
END $$;
COMMIT;
