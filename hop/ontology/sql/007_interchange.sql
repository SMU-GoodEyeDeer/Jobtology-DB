-- Native PostgreSQL JSON-LD serialization of a sealed release. No model or parser calls.
BEGIN;
CREATE OR REPLACE FUNCTION ontology.interchange_node_iri_v1(id text) RETURNS text
LANGUAGE sql IMMUTABLE STRICT AS $$
 SELECT 'urn:jobtology:node:'||encode(sha256(convert_to(id,'UTF8')),'hex')
$$;

CREATE OR REPLACE FUNCTION ontology.interchange_value_v1(value jsonb) RETURNS jsonb
LANGUAGE plpgsql IMMUTABLE STRICT AS $$
DECLARE result jsonb;
BEGIN
 CASE jsonb_typeof(value)
 WHEN 'array' THEN
  SELECT jsonb_build_object('@list',coalesce(jsonb_agg(ontology.interchange_value_v1(x) ORDER BY ord),'[]'))
  INTO result FROM jsonb_array_elements(value) WITH ORDINALITY a(x,ord);
  RETURN result;
 WHEN 'object' THEN RAISE EXCEPTION 'INTERCHANGE_EXPECTS_FLAT_PROPERTIES';
 WHEN 'number' THEN
  RETURN jsonb_build_object('@value',value#>>'{}','@type',CASE
   WHEN (value#>>'{}') ~ '^-?(0|[1-9][0-9]*)$' THEN 'xsd:integer' ELSE 'xsd:decimal' END);
 WHEN 'null' THEN RETURN jsonb_build_object('@value',value,'@type','@json');
 ELSE RETURN value;
 END CASE;
END $$;

CREATE OR REPLACE FUNCTION ontology.interchange_properties_v1(properties jsonb,mapping jsonb) RETURNS jsonb
LANGUAGE plpgsql IMMUTABLE STRICT AS $$
DECLARE result jsonb:='{}'; item record; term text;
BEGIN
 FOR item IN SELECT * FROM jsonb_each(properties) LOOP
  term:=coalesce(mapping->'properties'->>item.key,'jtf:'||encode(convert_to(item.key,'UTF8'),'hex'));
  result:=result||jsonb_build_object(term,ontology.interchange_value_v1(item.value));
 END LOOP;
 RETURN result||jsonb_build_object('nativePropertyKey',jsonb_build_object('@list',
  coalesce((SELECT jsonb_agg(k ORDER BY k COLLATE "C") FROM jsonb_object_keys(properties) k),'[]')));
END $$;

CREATE OR REPLACE FUNCTION ontology.interchange_check_v1(choice text,preview boolean DEFAULT false)
...
BEGIN
 ...
 SELECT artifacts->'mappings/hop-v1.json' INTO mapping FROM ontology.interchange_contract WHERE version='hop-ontology-interchange-v1';
...
CREATE OR REPLACE FUNCTION ontology.export_jsonld_v1(choice text DEFAULT NULL,preview boolean DEFAULT false)
...
BEGIN
 id:=ontology.interchange_check_v1(choice,preview);
 SELECT * INTO r FROM ontology.corpus_release WHERE release_id=id;
 SELECT * INTO contract FROM ontology.interchange_contract WHERE version='hop-ontology-interchange-v1';
 mapping:=contract.artifacts->'mappings/hop-v1.json';
